const RevealRemoteZoom = () => {
  ...
  function dispatchEnableZoom(focus) {
    if (!isValidFocus(focus)) {
      console.warn("invalid focus parameter for dispatchEnableZoom()");
      return;
    }
  ...
  function getRelativeLocation({ event, element }) {
    ...
    const x = Math.round(
      ((event.clientX - elementLocation.left) * 100) /
        (elementLocation.right - elementLocation.left)
    );
    const y = Math.round(
      ((event.clientY - elementLocation.top) * 100) /
        (elementLocation.bottom - elementLocation.top)
    );
  ...
  function applyZoom(focus) {
    if (focus === null) {
      ...
      if (!isValidFocus(focus)) {
        console.warn("invalid focus parameter for applyZoom()");
        return;
      }

      const transform =
        "scale(2) translate(" + (50 - focus.x) + "%, " + (50 - focus.y) + "%)";
  ...
  return {
    ...
    setCurrentZoom: (focus) => {
      if (focus === null) {
        ...
        if (!isValidFocus(focus)) {
          console.warn("invalid focus parameter for setCurrentZoom()");
          return;
        }
